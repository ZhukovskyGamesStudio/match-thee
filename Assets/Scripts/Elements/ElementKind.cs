...
public enum ElementKind {
    ...
    // Замок: стены и окно — обычные элементы (толкаются, собираются в ряды), пол — как трава.
    CastleWall,
    CastleWall2,
    CastleWall3,
    CastleWindow,
    ...
    // Рельеф многоуровневого мира: низина и плато на поверхности, обрыв между ними и пологий склон наверх.
    // Пещера — отдельный уровень под плато: снаружи её не видно, внутрь ведут вход в обрыве и лестницы.
    Lowland,
    Highland,
    Cliff,
    Slope,
    ...
    // Не рисуется на карте: вью само кладёт борт поверх земли по форме рельефа.
    Ledge, // стена обрыва со свесом и тенью под нависающей травой
}
